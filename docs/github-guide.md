...
> [!CAUTION]
> **这一步非常重要！每次开始工作前都必须执行 `git pull origin main`。**
>
> 如果你跳过 pull、直接在旧代码上改，提交后会把别人已经合并的新代码覆盖掉，导致别人的工作白做。**这是团队协作中最常见也最严重的事故。**
>
> 完整流程只有三步：① pull 最新 → ② 建分支改代码 → ③ push + 交 PR。缺一不可。
...
> [!TIP]
> 如果提交信息写错了，用 `git commit --amend -m "新的正确信息"` 修改。
